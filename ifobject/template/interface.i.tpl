...
{swrap 75 "        "}virtual unsigned int getNum{if prop.element.plural == ""}{$prop.element.name|uppercase(1)}s{else}{$prop.element.plural|uppercase(1)}{/if}() const;{/swrap}
{swrap 75 "        "}virtual {$prop.element.type} get{if prop.element.plural == ""}{$prop.element.name|uppercase(1)}s{else}{$prop.element.plural|uppercase(1)}{/if}({if prop.key.accessType != ""}{$prop.key.accessType}{else}{$prop.key.type}{/if} elementKey) const;{/swrap}
{if prop.readOnly != "true"}{if prop.hideImpl != "true"}
{swrap 75 "        "}virtual std::map<{$prop.key.type}, {$prop.element.type}>& get{$prop.name|uppercase(1)}();{/swrap}{/if}
...
{swrap 75 "        "}{$func.spec} {$func.type} {$func.name}({foreach prm in func.param}{$prm.type} {$prm.name}{if prm.default != ""} = {$prm.default}{/if}{first}, {/first}{mid}, {/mid}{/foreach}){if func.const == "true"} const{/if}{if func.throw != ""} throw({if func.throw != "<none>"}{$func.throw}{/if}){/if}{if func.pureVirtual == "true"} = 0{/if};{/swrap}{/if}{/foreach}{if enableUpcast == 1}
{swrap 75 "		"}static {foreach ns in namespace}{$ns.name}::{/foreach}{$class.name}* upcast(Ionflux::ObjectBase::IFObject* other);{/swrap}{/if}{if enableSerialize == 1}
        virtual bool serialize(std::string& target) const;
        virtual int deserialize(const std::string& source, int offset = 0);{/if}{if enablePersistence == 1}
        virtual void update();
        virtual int getPersistentID();
        virtual bool setFromID(int objectID);{foreach prop in property.protected}{if prop.persistentInit == "true"}{ref declarePersistentInitFunc}{/if}{/foreach}{foreach prop in property.private}{if prop.persistentInit == "true"}{ref declarePersistentInitFunc}{/if}{/foreach}{/if}{foreach prop in property.private}{ref createPropertyAccessorDecl}{/foreach}{foreach prop in property.protected}{ref createPropertyAccessorDecl}{/foreach}{if enablePersistence == 1}{$prop.name = "database"}{$prop.desc = "Database"}{$prop.type = class.persistence.namespace + "::" + class.persistence.database + "*"}{$prop.setFromType = class.persistence.namespace + "::" + class.persistence.database + "*"}{ref createPropertyAccessorDecl}{/if}{foreach sig in signal}{foreach ins in sig.instance}
