...
bool encode(const std::string& source, std::string& target)
{
	...
	int encodedLength = e.encode(source.c_str(), source.size(), buffer);
	target.assign(buffer, encodedLength);
	delete[] buffer;
	return true;
}
...
bool decode(const std::string& source, std::string& target)
{
	...
	int decodedLength = d.decode(source.c_str(), source.size(), buffer);
	target.assign(buffer, decodedLength);
	delete[] buffer;
	return true;
}
