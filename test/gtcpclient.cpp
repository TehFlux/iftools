 ...
 * mail@ionflux.org
 * --------------------------------------------------------------------------
 * gtcpclient.cpp                  Generic TCP client test (Glib based).
 * ==========================================================================
 * 
...
int main(int argc, char* argv[])
{
	...
	GlibMultiplexer iomp(argc, argv);
	TCPClient client(true, &iomp);
	signal(SIGINT, GlibMultiplexer::shutdownHandler);
	signal(SIGTERM, GlibMultiplexer::shutdownHandler);
	client.getLog().setVerbosityLevel(Reporter::VL_DEBUG_INSANE);
...
/** \file gtcpclient.cpp
 * \brief Generic TCP client test (Glib based).
 */
